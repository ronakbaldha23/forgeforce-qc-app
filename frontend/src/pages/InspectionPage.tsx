...
const APPROVER_ROLES = ['quality_manager', 'admin']

export function InspectionPage() {
  const { inspectionId } = useParams()
  const id = Number(inspectionId)
  const { user } = useAuth()
  const { inspection, isLoading, error, updateItemResult, applyInspection } = useInspection(id)
  ...
  const isApproved = inspection.approved_at !== null
  const canApprove = !isDraft && !isApproved && !!user && APPROVER_ROLES.includes(user.role)
  ...
  async function handleApprove() {
    ...
    try {
      const approved = await inspectionsApi.approve(id)
      applyInspection(approved)
    } catch (err) {
      setApproveError(err instanceof HttpError ? err.message : 'Could not approve inspection.')
    } finally {
  ...
  return (
    <div>
      ...
      <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
        ...
        >
          {isDraft ? 'In progress' : isApproved ? 'Approved' : 'Submitted'}
        </Badge>
      </div>
      <p className="text-sm text-muted-foreground">
        ...
        {inspection.approved_at &&
          ` · Approved by ${inspection.approved_by} on ${new Date(inspection.approved_at).toLocaleString()}`}
      </p>
      ...
      {canApprove && (
        <div className="mt-6">
          {approveError && <p className="mb-2 text-sm text-destructive">{approveError}</p>}
          <Button
            type="button"
            onClick={handleApprove}
            disabled={isApproving}
            className="h-14 w-full bg-indigo-600 text-base hover:bg-indigo-700"
          >
            {isApproving && <Loader2 className="animate-spin" />}
            {isApproving ? 'Approving…' : 'Approve inspection'}
          </Button>
        </div>
      )}
